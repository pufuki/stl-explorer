...
containers::ModuleInfo MapModule::info() const {
    return {
        .name = "std::map",
        .header_file = "<map>",
        .description = "Sorted associative container (Red-Black tree)"
    };
}

void MapModule::reset() {
    data_.clear();
...
void MapModule::resetWithInput(const std::string& input) {
...
Element MapModule::renderFullUI() const {
    auto& theme = utils::Theme::instance();
    ...
    return vbox({
        text("std::map") | bold | color(theme.colors().primary) | center,
        text("Sorted Associative Container (Red-Black Tree)") | dim | center,
        separator(),
        text("Key:Value pairs (auto-sorted by key):") | dim | center,
        ...
        separator(),
        text("insert/find/erase: O(log n) | Keys always sorted") | dim | center,
...
void MapModule::insert(int key, int value) {
    data_[key] = value;
    highlight_key_ = key;
    current_operation_ = "insert(" + std::to_string(key) + ", " + std::to_string(value) + ")";
}

void MapModule::erase(int key) {
...
ftxui::Component MapModule::getComponent() {
    return component_;
}
